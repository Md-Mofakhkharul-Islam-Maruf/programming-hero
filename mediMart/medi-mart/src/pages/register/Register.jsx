...
// import { AuthContext } from '../../contexts/AuthContext'; // Assuming you're using context
// import { updateProfile } from 'firebase/auth'; // If you're using Firebase
...
//   const from = location.state?.from?.pathname || '/';
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Optional: Upload photo to server/cloud and get back a photoURL
      const photoURL = URL.createObjectURL(photoFile); // Temporary preview URL

      await updateProfile(user, {
        displayName: name,
        photoURL: photoURL // Replace with uploaded URL if needed
      });

      // Save additional user info like role to your DB here (if applicable)

      await logOut();
      Swal.fire({
        icon: 'success',
        title: 'Registered Successfully',
        text: 'Profile updated. Please login.',
      }).then(() => navigate('/login'));
